use std::{fs::File, io::{BufWriter, Write}, path::{Path, PathBuf}, usize};
...
fn write_ldscript(raw: &[Segment], out: &Path) -> Result<()> {
...
fn seg_write<W: Write>(mut out: W, seg: &Segment, held: Holder) -> Result<()> {
    let begin_seg = |out: &mut W| writedoc!(out, "
    ...
    let begin_noload = |out: &mut W| { writedoc!(out, "
    ...
    let ovl_subarea = |out: &mut W, part: &str, loc: &str| {
        writeln!(out, "{sp}{sp}_{name}{part}{loc} = ABSOLUTE(.);", sp = INDENT, name = seg.name, part = part, loc = loc)
    };
    ...
    let addr_seg = |out: &mut W| { match seg.load {
    ...
    let system_noload = |out : &mut W, ss| -> Result<()> {
        ...
        writeln!(out, "{sp}_{name}SegNoloadSizeLo = SIZEOF(.{name}.noload) & 0xFFFF;", sp = INDENT, name = seg.name)?;
        writeln!(out)?;
        Ok(())
    };
