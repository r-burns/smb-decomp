...
void func_8001D610(struct Vec3f *out, struct Vec3f *ctrl, f32 s, f32 t) {
    ...
    out->x = ctrl[0].x*BASIS1(t,s) + 
        ctrl[1].x*BASIS2(t,s) + 
        ctrl[2].x*BASIS3(t,s) + 
        ctrl[3].x*BASIS4(t,s);

    out->y = ctrl[0].y*BASIS1(t,s) + 
        ctrl[1].y*BASIS2(t,s) + 
        ctrl[2].y*BASIS3(t,s) + 
        ctrl[3].y*BASIS4(t,s);

    out->z = ctrl[0].z*BASIS1(t,s) + 
        ctrl[1].z*BASIS2(t,s) + 
        ctrl[2].z*BASIS3(t,s) + 
        ctrl[3].z*BASIS4(t,s);
}
...
void func_8001D754(struct Vec3f *out, struct Vec3f *ctrl, f32 s, f32 t) {
    ...
    sX = ctrl[0].x*BASIS1(t,s) + ctrl[1].x*BASIS2(t,s) + ctrl[2].x*BASIS3(t,s) + ctrl[3].x*BASIS4(t,s);
    sY = ctrl[0].y*BASIS1(t,s) + ctrl[1].y*BASIS2(t,s) + ctrl[2].y*BASIS3(t,s) + ctrl[3].y*BASIS4(t,s);
    sZ = ctrl[0].z*BASIS1(t,s) + ctrl[1].z*BASIS2(t,s) + ctrl[2].z*BASIS3(t,s) + ctrl[3].z*BASIS4(t,s);
        ...
        if (t < 1.0f) {
            ...
            switch (params->kind) {
                ...
                case HAL_INTERP_BEZIER:
                    func_8001D8E0(out, params->unk08 + targetFrame, t);
                    break;
    ...
    if (!(t < 0.0f) && !(t > 1.0f)) {
        ...
        switch (params->kind) {
            ...
            case HAL_INTERP_BEZIER_S3:
                ...
                break;
            case HAL_INTERP_BEZIER:
                func_8001DA40(out, params->unk08 + targetFrame, t);
                break;
            case HAL_INTERP_CATROM:
